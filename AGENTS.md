...
- 每次完成并验收一个用户请求或可独立验证的任务增量后，自动形成有意、可追溯的 commit，并推送到现有 GitHub 私密仓库的 `main` 分支，无需再次询问。
- 推送前必须确认测试或对应 validator 通过、工作树不存在无关改动、远端仍为预期仓库。
- 若验证失败、存在无法归属的用户改动、认证失效、远端冲突或推送被拒绝，禁止强推；保留本地结果并向用户报告。
