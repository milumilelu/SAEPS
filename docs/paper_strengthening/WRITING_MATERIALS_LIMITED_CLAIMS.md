# SAEPS 论文补强写作素材：Limited-Claims 路线
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Burgers | 15 | 12 | 12 | 12/12 | 27.727 | 0.075032 | 27.636 | 0.00024414 |
| Allen–Cahn | 10 | 9 | 9 | 9/9 | 20.373 | 0.27857 | 20.006 | 0.0019531 |
...
|---|---:|---:|---|
| Burgers | 27.727 | 0.075032 | 误差大幅下降，但仍不是零误差；仅限声明的 exact-reference comparison |
| Allen–Cahn | 20.373 | 0.27857 | 独立 scalar replication 支持相同方向，但误差仍显著非零 |
...
- 新增 planned runs：55；terminal records：55；PASS：43；checkpoint-invalid：2；solver failures：10。
- exact-Hessian noise/sparsity anchors：14/15 binding-valid，全部有效 anchor 的 SAEPS/raw paired direction 有利于 SAEPS。
...
- 最低有效 cell 为 `noise=0.01_fraction=0.25`，只有 2/5 valid。
...
- 最大 state dimension：(n_	heta=100001)；最大 residual dimension：(m=3413)。
- residual-scalability grid：27/27 verified solves。
- P8 cost-only median：training 3.7659 s；SAEPS 5.221 s；reoptimized profile 9.1249 s。
- paired reoptimized-profile/SAEPS ratio：2.0483；ratio of medians：1.7477。
- median CG total iterations：549；median JVP count：556；median VJP count：555。
...
|---|---:|---:|---:|---:|---:|---|
| P5 scalar confirmation | 10 | 1 | 1 | 8 | 0 | `PARTIALLY_SUPPORTED` |
| P6 two-parameter confirmation | 10 | 0 | 4 | 2 | 4 | `FAIL` |
...
|---:|:---:|:---:|---:|---:|
| 200 | true | false | 0.794058 | 0.0984479 |
| 201 | true | false | 1.73825 | 1.08349 |
| 202 | true | false | 0.272789 | 0.554042 |
| 203 | true | false | 0.101858 | 0.159299 |
| 204 | true | true | 0.0538669 | 0.0010568 |

汇总：planned 5，evaluable 5，profile-valid 1；scientific status `NOT_SUPPORTED`。所有 5 个 evaluable records 的 descriptive (D) 为正，但该比较不能替代 profile validity gate。
...
|---|---:|---:|---:|
| `all_common_scales` | 3 | 0/9 | 0.000346235 |
| `two_finest_scales` | 2 | 0/9 | 0.000114424 |
| `finest_pair_only` | 1 | 7/9 | 1.02576e-15 |
...
v2 已测试 center-consistent objective、tighter stationarity candidates、objective non-increase、Newton polish、basin finder 和 `H(h)` 对 `h^2` 的拟合。最佳完整 cohort：planned 6 records，所有 fit windows 的 pass count 均为 0；修改报告记录的最佳完整 cohort 只有 5/48 profile points。
...
| Source | SHA256 |
|---|---|
| `docs/evidence/v5_final_audit.json` | `236ac82f6f10809052b97d355fe1b1f5cc39c616404e0890f320f0f64e4c906c` |
| `docs/evidence/p2_submission_statistics.json` | `f2718c204d33e00f2abcbd0328acff2c219661343b1c4852b73cc1feb9fd5052` |
| `docs/evidence/v4_2_confirmation.json` | `99184fce55c8847482f3adc4cf7d5db74b97f8a99f3171a95ab8cd3f101ea718` |
| `docs/evidence/v4_4_allen_confirmation.json` | `4e0188bfbe3e204f6a6c8cf2cdbd16fa8f0c0223e5922e97e0c6c2d7e37470d6` |
| `docs/evidence/v4_8_robustness.json` | `56fb4dc8b5325c3b0a05feaec71e69140602d10316285fcba5be5b284104b212` |
| `docs/evidence/v5/V5_PROFILE_BRIDGE_REPORT.json` | `2b1e4c8ea227309a076caec6b8fb385a374d483448f530641365726e3bb8b2ea` |
| `docs/evidence/v5/V5_TWO_PARAMETER_CONFIRMATION_REPORT.json` | `73a5ffb112e7a9c630d9ae9e5579ce5ca718e5d7a46c53210dac5d7c810628c8` |
| `docs/evidence/v5/V5_FINITE_GAMMA_AUDIT.json` | `79854e514da0a117413498af264b9bb37c6cd7bac76aa7e966c7f298b14a5b59` |
| `docs/evidence/P5_ACCEPTANCE.json` | `5fe943f086d6b1abbd0b95fc50ccfad920061303ca23fa80a6c0d5a7ddcb5e63` |
| `docs/evidence/P6_ACCEPTANCE.json` | `c18f3f04aed40925ee3e080bc28f45e66c6277464db3f01f155657d6e5793e4c` |
| `docs/evidence/P7_ACCEPTANCE.json` | `f1ca566396fc211b6542202e0506f76d4938e99b9ca6e3f04f1fa463a30219c2` |
| `docs/evidence/P8_ACCEPTANCE.json` | `f94f319bb990635a6add41c9ac1e5e07f2048605577af50b396b17fec30bfb2d` |
| `docs/evidence/P9_ACCEPTANCE.json` | `b0e938a1cdb2e8025f2935df7678abafd63a8a817d37809e42c9e0f6b201e836` |
| `outputs/runs/paper_strengthening_v1/s2_profile_development/S2_DEVELOPMENT_SUMMARY.json` | `d7e6f572754f6a02ca28eff9a2b06d43fa6928572228c4428c7dfbde51db6ace` |
| `outputs/runs/paper_strengthening_v1/s2_profile_development/S2_FIT_WINDOW_SUMMARY.json` | `600f5a3a47b01554f508394b06d0d2a9cba111a2d85697a732b05793ceabc234` |
| `outputs/runs/paper_strengthening_v2_basin_newton/s2_profile_development/S2_V2_DEVELOPMENT_SUMMARY.json` | `efe6c305f2ff6ed0dc1ebb6444ef6c8323f6d953bc6febd84c8f15e4b01b64a4` |
| `docs/paper_strengthening/S2_FIT_WINDOW_DECISION.md` | `d969232f03228a51c1a6f1f6a471abc2498a126fc1e5885ea359a07968611eaf` |
| `docs/paper_strengthening/S2_CENTER_REFINEMENT_ANALYSIS.md` | `b741d6ae7f77abf71ff40a5b7338af5eba71b7df56b7277561e9e266d5911dfb` |
| `docs/paper_strengthening/S2_V2_MODIFICATION_REPORT.md` | `cc567b00de9f03ea0890abcfcde1b84cf5f06d039e6faf39d1b548a3393b432c` |
| `paper/revise/paper_numbers.json` | `be03f8192db9a70273fc6170f0647a2b343904b11782c596c4482b597035f040` |
