...
Use a predeclared relative path γ=γ_rel (s_{ref}), with γ_rel in ([10^{-8},10^{-6},10^{-4},10^{-2},1]). Define (s_{ref}) from the state GN spectrum in a fixed standard coordinate; record estimator, random seed and cost. If the smallest gamma fails its numerical solve, mark it unavailable rather than removing it. Do not select a gamma per test instance using parameter error.
