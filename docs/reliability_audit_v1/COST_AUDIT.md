...
The corrected RI-2 development pilot (`outputs/reliability_audit_v1/pilot/ri2_full_v5`) executed 24 cases once. Per-run manifests record training wall time, dtype, hardware, Adam/L-BFGS iteration budgets, Jacobian dimensions, gamma scale and raw matrix paths. Derived gamma-path analysis records its source checkpoints and cached Jacobians under the versioned `reliability_analysis/derived_jacobians/` directory.
