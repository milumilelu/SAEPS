...
## D-Q2-007 — Coverage route passes analytic feasibility screen

Date: 2026-09-22. Scope: development-only, read-only local-Gaussian surrogate.

Using 21 valid exact-block scalar records, the nominal 95% Wald coverage surrogate has median 0.328/0.285 for raw information on Allen--Cahn/Burgers, 0.917/0.946 for SAEPS and 0.955/0.958 for the parameter-block candidate. Four of 25 planned exact-block files remain excluded with saved invalid status. This supports proceeding to an end-to-end noisy-data refitting pilot, but it is not empirical coverage and gives no finite-sample guarantee. Evidence: `configs/paper_strengthening/coverage_surrogate_posthoc_v1.yaml`; `outputs/posthoc/paper_strengthening/coverage_surrogate_v1/COVERAGE_SURROGATE_REPORT.md`.
