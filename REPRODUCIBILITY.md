...
The historical inventory and V5 paper-artifact manifest were frozen from Windows-generated files whose recorded working-tree bytes include CRLF in early-phase JSON/CSV records and the seven manifested V5 publication artifacts, while Git stores those text blobs with LF normalization. A fresh CI checkout therefore restores CRLF only for those precisely identified files before hashing. This is an ephemeral checkout transformation: it does not modify, regenerate, recommit, or reinterpret any scientific record or artifact.
