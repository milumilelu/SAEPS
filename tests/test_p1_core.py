...
from saeps.solvers import conjugate_gradient, preconditioned_conjugate_gradient
...
def test_jacobi_pcg_solves_actual_spd_system() -> None:
    matrix = torch.tensor([[40.0, 2.0], [2.0, 3.0]], dtype=torch.float64)
    right = torch.tensor([1.0, 2.0], dtype=torch.float64)
    diagonal = torch.diagonal(matrix)
    result = preconditioned_conjugate_gradient(
        lambda vector: matrix @ vector,
        right,
        lambda vector: vector / diagonal,
        1.0e-12,
        10,
    )
    assert result.converged
