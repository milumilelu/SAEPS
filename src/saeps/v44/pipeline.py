"""One-shot Allen--Cahn external confirmation seed pipeline."""
...
def _initial_record(seed: int, digest: str, provenance: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "phase": "V4_4_ALLEN_CAHN_EXTERNAL_CONFIRMATION",
        "role": "CONFIRMATION",
...
def run_allen_confirmation_seed(
    seed: int,
    ...
    if specification["confirmation_authorized"] is not False:
        raise RuntimeError("locked confirmation flag must remain false; authorization is separate")
    if seed not in [int(value) for value in specification["development_seeds"]]:
        raise ValueError("seed is not authorized for Allen-Cahn confirmation")
    authorization_path = root / "configs/v4_4/EXECUTION_AUTHORIZATION.json"
    if not authorization_path.is_file():
        raise RuntimeError("Allen-Cahn confirmation is not authorized")
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    if authorization.get("state") != "AUTHORIZED_ONCE":
        raise RuntimeError("invalid Allen-Cahn confirmation authorization")
    if authorization.get("locked_config_sha256") != _sha256(Path(config_path)):
        raise RuntimeError("authorization/config hash mismatch")
    for source_name in ["source_development_freeze", "source_heldout_evidence"]:
        source = specification[source_name]
        if _sha256(root / source["path"]) != source["sha256"]:
            raise RuntimeError(f"protected v4.3 source changed: {source_name}")
    ...
    provenance = (
        ...
        else environment_provenance(root, runtime["dtype"], "cpu")
    )
    if provenance_override is None and provenance["git_dirty"]:
    ...
    try:
        ...
        _write(
            destination / "manifest.json",
            {
                ...
                "scientific_gate": "LOCKED_PENDING_AGGREGATION",
            },
        )
