...
def run_scalability_checkpoint(root: Path, seed: int) -> dict:
    ...
    passed = applied.solve.relative_residual <= float(specification["cg_acceptance"]) and (explicit_error is None or explicit_error <= float(specification["explicit_matrix_free_relative_tolerance"]))
    result = {"schema_version": 1, "phase": specification["phase"], "checkpoint": seed, "status": "PASS" if passed else "SOLVER_FAILURE", "scientific_claim_binding": False, "state_parameter_count": theta.numel(), "residual_count": jl.shape[0], "target_width": target_width, "function_preserving_padding": True, "gamma": gamma, "lambda_max_power_estimate": lambda_estimate, "cg_iterations": applied.solve.iterations, "verified_relative_residual": applied.solve.relative_residual, "explicit_relative_error": explicit_error, "JVP_count": dict(linearization.operation_counts).get("jvp_theta", 0), "VJP_count": dict(linearization.operation_counts).get("vjp_theta", 0), "training_seconds": checkpoint.elapsed_seconds, "solve_seconds": solve_seconds, "elapsed_seconds": time.perf_counter() - started, "provenance": provenance, "config_hash": config_hash(specification)}
